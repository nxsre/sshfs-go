...
sshfs-%-linux-amd64.tar.gz: test
	./release/build.sh
	mkdir sshfs-$*-linux-amd64
	mv sshfs sshfs-$*-linux-amd64/sshfs
	cp README.md LICENSE sshfs-$*-linux-amd64/
	tar -czvf $@ sshfs-$*-linux-amd64
	rm -rf sshfs-$*-linux-amd64
